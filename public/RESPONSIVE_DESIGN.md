...
### 논리연산자
지금 모바일 기기에 대한 반응형 디자인을 하고 있다.  
이때 발생하는 문제점이 하나 있는데 바로 테블릿 기기에서의 가로 모드이다.  
이때 이미지가 우리의 예상보다 커진다.  
너비는 미디어 쿼리 제한을 넘는데 높이는 아직 미디어 쿼리를 실행하기 부족하다.  
왜냐하면 가로 모드와 세로 모드 때문에 너비와 높이가 바뀌었기 때문이다.  
  
따라서 너비와 높이를 동시에 고려할 수 있다면 더 좋을 것 같다.  
너비 와 높이 두가지 기준을 모두 충족할때만 이미지를 늘리거나 미디어 쿼리를 적용하게 만들면 어떨까?  
물론 충분히 미디어 쿼리를 이용해서 높이를 지정할 수 있지만  
높이와 너비를 하나씩 따로 설정할 수는 없다.  
  
한 미디어 쿼리에 두가지 너비 와 높이 둘다 지정해야 한다.  
```
...
```
위와 같이 기존 조건에 and 연산자를 통해 새로운 조건을 추가 하였다.  
따라서 양쪽 너비 와 높이 조건을 만족해야지만 우리가 작성한 css 코드가 실행된다.  

```
@media (min-width: 40rem) , (min-height: 60rem){
}
```
and 말고 `,`로 조건을 연결하면 두 조건중 하나가 충족될때(or) 코드가 적용 된다.

## footer 배치
푸터가 항상 페이지 하단에 위치하도록 정의하려면 어떻게 해야할까??  
기본적으로 필요한 것은 상단의 헤더(header)입니다.  
그리고 하단의 푸터와 헤더 사이에는 메인 파트가 들어간다.  
main 요소의 높이를 100vh 또는 100%로 설정하고 거기서 내비게이션 바의 높이와 푸터가 들어갈 공간을 빼면 된다.  
그러면 푸터가 항상 페이지 맨 아래에 위치하게 할 설정을 할 수 있다.  
  
그러기 위해서는 먼저 main 요소가 있어야하고 계산식이 있어야 한다.
```
main {
